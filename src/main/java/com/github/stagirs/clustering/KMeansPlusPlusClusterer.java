...
import com.github.stagirs.clustering.distance.*;
...
public class KMeansPlusPlusClusterer<T extends Clusterable> extends Clusterer<T> {
    ...
    private Clusterable centroidOf(final Collection<T> points) {
        final TIntDoubleHashMap centroid = new TIntDoubleHashMap();
        for (final T p : points) {
            final TIntDoubleHashMap point = p.getPoint();
            for (int index : point.keys()) {
                centroid.adjustOrPutValue(index, point.get(index), point.get(index));
            }
        }
        for (int index : centroid.keys()) {
            centroid.put(index, centroid.get(index) / points.size());
        }
        return new SimpleClusterable(centroid);
    }
